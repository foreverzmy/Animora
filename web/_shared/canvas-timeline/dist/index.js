function normalizedFrame(frame, length, mode) {
    if (length < 1)
        throw new Error("A sequence must contain at least one frame");
...
export class FrameClock {
    ...
    requestId = 0;
    running = false;
    constructor(fps, onFrame) { this.fps = fps; this.onFrame = onFrame; }
    get currentFrame() { return Math.max(0, this.shownFrame); }
    start() { this.play(); }
    play() {
        if (this.running)
            return;
    ...
    pause() { this.running = false; cancelAnimationFrame(this.requestId); }
    seek(frame) {
...
export class CanvasTimelinePlayer {
    manifest;
    context;
    assetBase;
    clock;
    ...
    audio;
    wantedFrame = 0;
    constructor(options, manifest, assetBase) {
    ...
    static async create(options) {
        ...
        const player = new CanvasTimelinePlayer(options, await response.json(), new URL(".", response.url));
        ...
        if (query === null && options.autoplay !== false)
            player.play();
        return player;
    }
    get currentFrame() { return this.clock.currentFrame; }
    pause() { this.clock.pause(); this.audio?.pause(); }
    play() { this.clock.play(); void this.startAudio(); }
    async seek(frame) {
        const target = Math.max(0, Math.floor(frame));
        this.clock.pause();
        ...
        if (this.audio && Number.isFinite(this.audio.duration) && this.audio.duration > 0)
            this.audio.currentTime = (target / this.manifest.fps) % this.audio.duration;
    }
    async request(frame) {
        this.wantedFrame = frame;
        await this.ensureFrame(frame);
        if (frame !== this.wantedFrame)
            return;
        this.draw(frame);
        for (let offset = 1; offset <= this.lookaheadFrames; offset += 1)
            void this.ensureFrame(frame + offset);
    }
    draw(frame) {
    ...
    sequence(name) {
        ...
        if (!sequence)
            throw new Error(`Missing timeline sequence: ${name}`);
        return sequence;
    }
    load(asset) {
    ...
    async startAudio() {
        ...
        catch { /* Browsers retry on the first user gesture. */ }
    }
}
